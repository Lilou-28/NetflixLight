function initTrendingWeekSwiper() {
  const totalSlides = document.querySelectorAll("#trending-week-wrapper .swiper-slide").length;
  const canLoop = totalSlides > 3;

  new Swiper(".trending-week-swiper", {
    loop: canLoop,
    slidesPerView: 1.2,
    spaceBetween: 12,
    watchOverflow: true,
...
async function loadTrendingWeekMovies() {
  const wrapper = document.getElementById("trending-week-wrapper");
    if (!wrapper) {
    return;
  } 

    try {
    const response = await fetch("/trending/week", { credentials: "same-origin" });
    ...
    initTrendingWeekSwiper();
}

  document.addEventListener("DOMContentLoaded", loadTrendingWeekMovies);
